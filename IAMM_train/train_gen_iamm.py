...
class GenerationTrainer:
    def __init__(self, config: GenTrainingConfig):
        ...
        self.emo_head = EmotionHead(config)
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step, batch in pbar:
                with self.accelerator.accumulate(self.model):
                    # Forward
                    def quick_forward(input_ids: torch.Tensor):
                        output = self.model(
                            input_ids=input_ids, 
                    ...
                    ctx_loss, ctx_last_hidden_dim = quick_forward(batch['input_batch'])
                    sit_loss, sit_last_hidden_dim = quick_forward(batch['situation_batch'])
                    
                    sit_emo_logit = self.emo_head(sit_last_hidden_dim)
